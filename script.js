    ...
    if (e.isIntersecting) {
      ...
      if (e.target.closest('#skills')) {
        document.querySelectorAll('.bar span').forEach(span => {
          const width = span.style.width || span.getAttribute('data-width') || span.getAttribute('style').match(/width:\s*(\d+%)/);
          // read current inline width if any
          const computed = span.getAttribute('style').match(/width:\s*(\d+%)/);
          // animate to the declared width (already set in HTML style attributes)
          const targetW = span.style.width || span.getAttribute('data-target') || (computed ? computed[1] : '80%');
...
/* Responsive: simple menu toggle (optional) */
(function(){
  if(window.matchMedia && window.matchMedia('(max-width:900px)').matches){
    // Add mobile toggle
    const nav = document.getElementById('nav-links');
    // simple toggle when brand clicked (for dev speed). You can implement a hamburger if desired.
